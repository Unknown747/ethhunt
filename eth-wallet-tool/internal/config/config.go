...
type RPCConfig struct {
	Endpoints     []string          `yaml:"endpoints"`      // URL biasa tanpa auth
	EndpointsAuth []RPCEndpointAuth `yaml:"endpoints_auth"` // URL dengan custom header
...
// RPCEndpointAuth adalah endpoint RPC dengan custom HTTP headers (Infura, Alchemy, dll)
type RPCEndpointAuth struct {
	URL     string            `yaml:"url"`
...
type GeneratorConfig struct {
	...
	MnemonicPaths int    `yaml:"mnemonic_paths"` // jumlah address per seed (default 5)
}
...
func (c *Config) applyDefaults() {
	if len(c.RPC.Endpoints) == 0 {
		c.RPC.Endpoints = Default().RPC.Endpoints
	}
	...
	if c.Generator.MnemonicPaths <= 0 {
		c.Generator.MnemonicPaths = 5
	}
